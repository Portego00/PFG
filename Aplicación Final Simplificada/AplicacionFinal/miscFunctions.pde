...
void mousePressed() {
  ...
  //OPTION: Measurement
  if (b_ConnectDisconnect.isMouseOver())
  {
...
boolean ConnectionProcedure() {
  ...
  if (b_ConnectDisconnect.getState()) { //if true we are connected and we want to disconnect
    ...
    try {
      ...
      rect(40,135,290,90);
      fill(0);
      text("Establishing Connection to Lantier Board...", 45, 150);
      return true;
    } 
...
void MeasurementProcedure() {
  ...
  if (b_StartStopMeasure.getState()) { //if true the system is started and we want to stop it.
    ...
    if (nErrors==0) {
      ...
      measurementThread.setParameters(DEFstartFreq, DEFstepFreq, DEFstopFreq, DEFdelayBf, DEFnumRepMeasure, listRF, nRFouts);  
